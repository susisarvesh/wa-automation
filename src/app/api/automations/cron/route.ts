 ...
 * on a schedule (Vercel Cron / GitHub keep-warm) — requires
 * AUTOMATION_CRON_SECRET or CRON_SECRET via Bearer / x-cron-secret.
 *
...
export async function GET(request: Request) {
  if (!authorizeCron(request)) {
    ...
    return NextResponse.json(
      { error: configured ? 'Unauthorized' : 'cron not configured' },
